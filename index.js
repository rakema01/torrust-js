...
class Torrust {
		...
		return new Promise((resolve, reject) => {
			try {
				const tokenValidated =
					typeof this.token == "string" && this.token.split(".").length >= 3
						? this.token
						: "";
				https.get(
					this.getHTTPOptions(path, {
						Authorization: `Bearer ${tokenValidated}`,
					}),
					(res) => {
	...
	getTorrent(id) {
		return new Promise((resolve, reject) => {
			this.get(`/api/torrent/${id}`).then((res) => {
				parseJSON(res).then(resolve);
			}, reject);
		});
	}
	downloadTorrentFile(id, path) {
		return new Promise((resolve, reject) => {
			if (
				fs.existsSync(`${path}/${id}.torrent`) ||
				fs.existsSync(`${path}/${id}.torrent.downloading`)
			) {
				reject("Torrent already exists or is already downloading.");
				return;
			}
			this.get(`/api/torrent/download/${id}`).then((res) => {
	...
	getTorrents(page_size) {
		return new Promise((resolve, reject) => {
			this.get(`/api/torrents?page_size=${page_size}`).then((res) => {
				parseJSON(res).then(resolve);
			}, reject);
		});
	}
	verifyToken() {
		return new Promise((resolve, reject) => {
			this.getTorrents().then((torrents) => {
				this.getTorrent(torrents.results[0].torrent_id).then((torrent) => {
